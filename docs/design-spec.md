...
- **Edit (0.10.2):** `openEditor(item)` places the item sheet at `besidePosition(its width)`, top clamped to the window, and remembers it until the large card closes: pointerdown and Escape inside it are ignored by the large card. The large card is in the top layer, so a window can only avoid it by sitting beside it. Detached, the item sheet opens unpositioned in the main window.
