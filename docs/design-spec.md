...
- **Small cards** are 104px tall with the item image full-bleed. Name, training and type are overlaid in light text on a dark shade, which stays dark in both themes for contrast. The bottom button row holds the d20, plus a Worn toggle on armor cards, centred together; archive moved to the popover. Names wrap at word boundaries to two lines, then end with an ellipsis; the full name is in the tooltip and popover. Art that fails to load is hidden rather than shown as a broken image. The system's SVG icons are shown contained rather than cropped.
